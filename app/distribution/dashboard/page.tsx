    ...
    <div style={styles.qrPlaceholder}>
      <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
        <rect
          ...
          fill="none"
        />
        <rect x="6" y="6" width="10" height="10" rx="1" fill="#3D4B9C" />
        <rect
          x="28"
          y="2"
          width="18"
          ...
          fill="none"
        />
        <rect x="32" y="6" width="10" height="10" rx="1" fill="#3D4B9C" />
        <rect
          x="2"
          y="28"
          width="18"
          ...
          fill="none"
        />
        <rect x="6" y="32" width="10" height="10" rx="1" fill="#3D4B9C" />
        <rect x="28" y="28" width="4" height="4" fill="#3D4B9C" />
        <rect x="34" y="28" width="4" height="4" fill="#3D4B9C" />
        <rect x="40" y="28" width="6" height="4" fill="#3D4B9C" />
        <rect x="28" y="34" width="6" height="4" fill="#3D4B9C" />
        <rect x="36" y="34" width="4" height="4" fill="#3D4B9C" />
        <rect x="40" y="38" width="6" height="8" fill="#3D4B9C" />
        <rect x="28" y="40" width="10" height="6" fill="#3D4B9C" />
      </svg>
      <div
        style={{
          fontSize: 10,
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Show this QR to the <strong>{nextActor}</strong> to confirm receipt
          and complete the handover on the blockchain.
        </div>
        <div
          style={{
            display: "flex",
...
const DOC_OPTIONS: { label: string; description: string }[] = [
  {
    label: "Beneficiary Receipt",
    description: "Signed receipt from beneficiary",
  },
  { label: "Handover Photo", description: "Photo proof of item handover" },
  {
    label: "Stock Register Entry",
    description: "Official stock distribution register",
  },
];
...
export default function DistributorDashboardPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("name", distributorName);
      formData.append("node_id", distributorName);
    ...
    <div style={styles.shell}>
      ...
      <aside style={styles.sidebar}>
        <div style={{ marginBottom: "24px" }}>
          ...
          <div style={styles.logoSub}>Subsidy Distribution</div>
        </div>
      ...
      <main style={styles.main}>
        <header style={styles.header}>
          ...
          <p style={styles.sub}>
            {activeView === "overview"
              ? "Manage subsidy stock and upload applicant verification for Admin approval."
              : "Fill in your details, attach distribution documents, and submit for Admin verification."}
          </p>
        </header>

        <section style={styles.card}>
          <h2 style={styles.h2}>Scan Transporter QR</h2>
          <p style={styles.text}>Scan the transporter's QR (camera or file) to verify goods.</p>
              ...
              onResult={(res) => {
                try {
                  ...
                  if (data.nextStage !== "distributor_handover") {
                    throw new Error(`Invalid Role: This QR is not intended for the distributor node. Expected: distributor_handover, got: ${data.nextStage}`);
                  }
        ...
        <>
          <section style={styles.card}>
            <h2 style={styles.h2}>Current Batch Status</h2>
            <div style={styles.grid}>
              <Info
                label="Municipality"
                value="Ward No. 4 - Central Office"
              />
              <Info label="Subsidy Item" value="Electric Irrigation Pumps" />
              <Info label="Total Stock" value="120 Units" />
              <Info label="Distributed" value="48 Units (40%)" />
              <Info label="Admin Verified" value="42/48 Submissions" />
              <Info label="Pending Verification" value="6 Required" />
            </div>
          </section>
        </>
        ...
        {activeView === "submit" && (
          <section style={styles.card}>
            ...
            <p style={styles.text}>
              Fill in your distributor details and attach verification documents
              — beneficiary receipts, handover photos, and stock register
              entries — for Admin approval.
            </p>
...
function SubmissionStatusCard({
  ...
  return (
    ...
    >
      ...
      {/* ── QR: shown when approved and qrData exists — for Beneficiary to scan ── */}
        ...
        >
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: "#555", lineHeight: 1.5 }}>
              Your documents have been approved. Show this QR to the{" "}
              <strong>Beneficiary</strong> to complete the final delivery on the
              blockchain.
            </div>
            {submission.cid && (
              <div
...
const styles: Record<string, React.CSSProperties> = {
  ...
  qrPlaceholder: {
    border: "2px dashed rgba(61,75,156,0.3)",
    borderRadius: 8,
    background: "#f8faff",
    display: "flex",
    flexDirection: "column",
  ...
  scannerLine: {
    ...
    animation: "distributor-scan 2s ease-in-out infinite",
  },
  scannerHint: {
    color: "#fff",
    fontSize: 12,
